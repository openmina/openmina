...
impl<F: FieldWitness> CheckedNat<F, 64> for CheckedN<F> {
    ...
    // REVIEW(dw): N is generated by a macro. It is simply a 64 bits unsigned value.
    // REVIEW(dw): check if it is actually 64bits unsigned values as in OCaml,
    // or if it is 62 bits value (1 bit reserved for GC + 1 for the sign as no
    // u64 in Caml)
