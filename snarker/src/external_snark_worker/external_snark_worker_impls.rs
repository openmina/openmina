...
use mina_p2p_messages::v2::{
    CurrencyFeeStableV1, MinaStateProtocolStateBodyValueStableV2, NonZeroCurvePoint,
...
pub fn available_job_to_snark_worker_spec<F>(
...
where
    F: Fn(StateBodyHash) -> MinaStateProtocolStateBodyValueStableV2,
{
    let instances = match job {
        OneOrTwo::One(v) => SnarkWorkerWorkerRpcsVersionedGetWorkV2TResponseA0Instances::One(
            with_merged_statement(v, protocol_body),
        ),
        OneOrTwo::Two((v1, v2)) => {
            SnarkWorkerWorkerRpcsVersionedGetWorkV2TResponseA0Instances::Two((
                with_merged_statement(v1, protocol_body),
                with_merged_statement(v2, protocol_body),
            ))
        }
    };

    SnarkWorkerWorkerRpcsVersionedGetWorkV2TResponse(Some((
    ...
    F: Fn(StateBodyHash) -> MinaStateProtocolStateBodyValueStableV2,
{
    match job {
        AvailableJobMessage::Base(TransactionSnarkScanStateTransactionWithWitnessStableV2 {
            ...
            let protocol_state_body = protocol_body(state_hash.1);
        ...
        AvailableJobMessage::Merge {
            ...
            let merged_stmt = ledger_stmt1.merge(&ledger_stmt2).unwrap();
