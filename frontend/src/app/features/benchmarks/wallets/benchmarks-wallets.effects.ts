...
export class BenchmarksWalletsEffects extends MinaRustBaseEffect<BenchmarksWalletsActions> {
  ...
  constructor(
    ...
    this.getWallets$ = createEffect(() =>
      this.actions$.pipe(
        ofType(BENCHMARKS_WALLETS_GET_WALLETS, BENCHMARKS_WALLETS_CLOSE),
        this.latestActionState<
          BenchmarksWalletsGetWallets | BenchmarksWalletsClose
        >(),
        switchMap(({ action }) =>
          action.type === BENCHMARKS_WALLETS_CLOSE
            ? EMPTY
            : this.benchmarksService.getAccounts().pipe(
                switchMap(payload => {
                  ...
                  actions.push({
                    type: BENCHMARKS_WALLETS_GET_WALLETS_SUCCESS,
                    payload,
                  });
                  return actions;
                }),
    ...
    this.sendTxs$ = createEffect(() =>
      this.actions$.pipe(
        ofType(BENCHMARKS_WALLETS_SEND_TXS),
        this.latestActionState<BenchmarksWalletsSendTxs>(),
        switchMap(({ state }) =>
          this.benchmarksService.sendTransactions(
            state.benchmarks.wallets.txsToSend,
          ),
        ),
    ...
    this.sendTxSuccess$ = createEffect(() =>
      this.actions$.pipe(
        ofType(
          BENCHMARKS_WALLETS_SEND_TX_SUCCESS,
          BENCHMARKS_WALLETS_SEND_ZKAPPS_SUCCESS,
    ...
    this.sendZkApps$ = createEffect(() =>
      this.actions$.pipe(
        ofType(BENCHMARKS_WALLETS_SEND_ZKAPPS),
        this.latestActionState<BenchmarksWalletsSendTxs>(),
        switchMap(({ state }) =>
          this.zkService.sendZkApp(state.benchmarks.wallets.zkAppsToSend),
        ),
        map(payload => ({
          type: BENCHMARKS_WALLETS_SEND_ZKAPPS_SUCCESS,
          payload,
        })),
      ),
    );
