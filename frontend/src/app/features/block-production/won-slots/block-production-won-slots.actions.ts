...
const type = <T extends string>(type: T) =>
  createType(BLOCK_PRODUCTION_PREFIX, 'Won Slots', type);
...
const getSlotsSuccess = createAction(
  type('Get Slots Success'),
  props<{
...
const changeFilters = createAction(
  type('Change Filters'),
  props<{ filters: BlockProductionWonSlotsFilters }>(),
);
const setActiveSlot = createAction(
  type('Set Active Slot'),
  props<{ slot: BlockProductionWonSlotsSlot }>(),
);
const setActiveSlotNumber = createAction(
  type('Set Active Slot Number'),
  props<{ slotNumber: number }>(),
);
const sort = createAction(
  type('Sort'),
  props<{ sort: TableSort<BlockProductionWonSlotsSlot> }>(),
);
