...
function sortSlots(
  node: BlockProductionWonSlotsSlot[],
  tableSort: TableSort<BlockProductionWonSlotsSlot>,
): BlockProductionWonSlotsSlot[] {
...
function filterSlots(
  ...
  filters: BlockProductionWonSlotsState['filters'],
): BlockProductionWonSlotsSlot[] {
  return slots.filter(slot => {
    if (
      (filters.accepted &&
        slot.status === BlockProductionWonSlotsStatus.Canonical) ||
      (filters.orphaned &&
        slot.status === BlockProductionWonSlotsStatus.Orphaned) ||
      (filters.discarded &&
        slot.status === BlockProductionWonSlotsStatus.Discarded) ||
      slot.active ||
    ...
    return (
      (filters.upcoming && !slot.status) ||
      slot.status === BlockProductionWonSlotsStatus.Scheduled
    );
  });
}
