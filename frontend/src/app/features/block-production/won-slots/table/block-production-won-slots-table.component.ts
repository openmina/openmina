...
export class BlockProductionWonSlotsTableComponent
  extends MinaTableRustWrapper<BlockProductionWonSlotsSlot>
  implements OnInit
{
  ...
  protected readonly tableHeads: TableColumnList<BlockProductionWonSlotsSlot> =
    [
      ...
      { name: 'height' },
      {
        name: 'global slot',
        sort: 'globalSlot',
        tooltip: 'Global slot since hard fork',
      },
  ...
  protected override setupTable(): void {
    this.table.gridTemplateColumns = isDesktop()
      ? [210, 140, 90, 142, 120, 120, 120, 124, 159, 150]
      : [150, 100, 90, 130, 100, 105, 100, 114, 139, 130];
    ...
    this.table.trackByFn = (_: number, item: BlockProductionWonSlotsSlot) =>
      item.message +
      item.slotTime +
      item.transactionsTotal +
      item.snarkFees +
      item.coinbaseRewards +
      item.txFeesRewards;
  }
  ...
  private listenToNodesChanges(): void {
    this.select(
      BlockProductionWonSlotsSelectors.filteredSlots,
  ...
  private scrollToElement(): void {
    const finder = (node: BlockProductionWonSlotsSlot) =>
      node.globalSlot.toString() === this.fromRoute;
  ...
  protected override onRowClick(
    ...
    if (this.table.activeRow?.globalSlot !== slot?.globalSlot || isRealClick) {
      ...
      this.router.navigate(
        [Routes.BLOCK_PRODUCTION, Routes.WON_SLOTS, slot.globalSlot],
