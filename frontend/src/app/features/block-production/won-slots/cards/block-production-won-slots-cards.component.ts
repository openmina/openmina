...
export class BlockProductionWonSlotsCardsComponent
  extends StoreDispatcher
  implements OnInit
{
  card1: { nextWonSlot: string; slot: number } = {
    nextWonSlot: '-',
    slot: null,
  };
  card2: { wonSlots: number; slotsUsed: number } = {
    wonSlots: null,
    slotsUsed: null,
  };
  card3: { acceptedBlocks: number; lastBlockTime: string } = {
    acceptedBlocks: null,
    lastBlockTime: null,
  };
  card4: { epochProgress: string; endIn: string } = {
    epochProgress: '-',
    endIn: null,
  };
  card5: { publicKey: string; totalRewards: string } = {
    publicKey: null,
    totalRewards: null,
  };
  ...
  private listenToEpoch(): void {
    this.select(
      ...
      (epoch: BlockProductionWonSlotsEpoch) => {
        const epochEndTime = this.addMinutesToTimestamp(
          epoch.currentTime / ONE_BILLION,
          (epoch.end - epoch.currentGlobalSlot) * 3,
        );
        this.card4.endIn = getTimeDiff(epochEndTime * ONE_THOUSAND).diff;
        this.card4.epochProgress =
          Math.floor(
            ((epoch.currentGlobalSlot - epoch.start) /
    ...
    this.select(
      ...
      (slots: BlockProductionWonSlotsSlot[]) => {
        const nextSlot = slots.find(
          s =>
            s.status === BlockProductionWonSlotsStatus.Scheduled || !s.status,
        );
        if (nextSlot) {
        ...
        this.card2.slotsUsed = slots.filter(s =>
          [
        ...
        this.card3.acceptedBlocks = slots.filter(
          s => s.status === BlockProductionWonSlotsStatus.Canonical,
        ).length;
        this.card3.lastBlockTime = getTimeDiff(
          lastItem(
            slots.filter(
              s => s.status === BlockProductionWonSlotsStatus.Canonical,
            ),
        ...
        this.card5.totalRewards = slots
          .filter(s =>
            [BlockProductionWonSlotsStatus.Canonical].includes(s.status),
          )
          .map(s => s.coinbaseRewards + s.txFeesRewards)
          .reduce((a, b) => a + b, 0)
          .toFixed(0);

        this.card5.totalRewards = isNaN(+this.card5.totalRewards)
          ? '0'
          : this.card5.totalRewards;
        this.detect();
      },
