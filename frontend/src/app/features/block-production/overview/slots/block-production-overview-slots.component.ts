...
import { BlockProductionOverviewFilters } from '@shared/types/block-production/overview/block-production-overview-filters.type';
...
export class BlockProductionOverviewSlotsComponent
  ...
  implements AfterViewInit
{
  ...
  private slotsGroup: d3.Selection<
    SVGGElement,
    unknown,
    HTMLElement,
    undefined
  ...
  private listenToActiveSlot(): void {
    this.select(
      BlockProductionOverviewSelectors.activeSlot,
      (slot: BlockProductionOverviewSlot) => {
        this.activeSlot = slot;
        this.slotsGroup
          .selectAll<SVGRectElement, BlockProductionOverviewSlot>('rect')
          .filter(
            (d: BlockProductionOverviewSlot) =>
              d.slot === this.activeSlot?.slot,
          )
  ...
  private listenToFilters(): void {
    this.select(
      BlockProductionOverviewSelectors.filters,
  ...
  private listenToElWidthChange(): void {
    ...
    fromEvent(this.svgHolder.nativeElement, widthChange)
      .pipe(
  ...
  private addSvgWizard(): void {
    ...
    this.svg = d3
      .select(this.svgHolder.nativeElement)
      .append('svg')
  ...
  private initSlots(): void {
    ...
    this.slotsGroup
      ...
      .attr('x', this.setX(slotsPerRow))
      .attr('y', this.setY(slotsPerRow))
      .attr('fill', (d: BlockProductionOverviewSlot) =>
        BlockProductionOverviewSlotsComponent.getSlotColor(d, this.filters),
      )
      .on('click', (_: MouseEventWithTarget, d: BlockProductionOverviewSlot) =>
        this.onSlotClick(d),
      )
      .on(
        'mouseover',
        (event: MouseEventWithTarget, d: BlockProductionOverviewSlot) =>
  ...
  private setY(
    slotsPerRow: number,
  ): (d: BlockProductionOverviewSlot, i: number) => number {
    return (_: BlockProductionOverviewSlot, i: number) =>
      Math.floor(i / slotsPerRow) * (this.slotHeight + this.slotMargin);
  }
  ...
  private setX(
    ...
    return (_: BlockProductionOverviewSlot, i: number) =>
      (i % slotsPerRow) * (this.slotWidth + this.slotMargin);
  }
  ...
  private onSlotClick(slot: BlockProductionOverviewSlot): void {
    this.zone.run(() => {
      this.slotsGroup
        .selectAll<SVGRectElement, BlockProductionOverviewSlot>('rect')
        .filter(
          (d: BlockProductionOverviewSlot) => d.slot === this.activeSlot?.slot,
        )
        .attr('stroke', undefined);

      this.dispatch2(
        BlockProductionOverviewActions.setActiveSlot({ slot: slot.slot }),
      );
      this.router.navigate(
        [
          Routes.BLOCK_PRODUCTION,
          Routes.OVERVIEW,
          this.activeEpoch.epochNumber,
          slot.slot,
        ],
  ...
  private mouseOverHandle(
    ...
    const svgHolderBoundingClientRect =
      this.svgHolder.nativeElement.getBoundingClientRect();
    ...
    selection.style('left', `${desiredLeft}px`).style('top', `${desiredTop}px`);
  }
  ...
  public static getSlotColor(
    ...
    if (d.canonical) {
      ...
      color =
        'special-selected-alt-1-' +
        (filters?.orphaned ? 'primary' : 'tertiary');
    } else if (d.missed) {
