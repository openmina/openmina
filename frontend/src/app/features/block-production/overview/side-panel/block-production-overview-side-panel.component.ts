...
export class BlockProductionOverviewSidePanelComponent
  ...
  implements OnInit
{
  ...
  private listenToActiveEpoch(): void {
    this.select(
      ...
      (epoch: BlockProductionOverviewEpoch) => {
        ...
        const startSlot = epoch.slots.find(
          slot => slot.globalSlot === epoch.details.slotStart,
        );
        const endSlot = epoch.slots.find(
          slot => slot.globalSlot === epoch.details.slotEnd,
        );
        this.extras = {
          ...
          epochStarted: startSlot
            ? toReadableDate(startSlot.time * ONE_THOUSAND, noMillisFormat)
            : '-',
          epochEnds: endSlot
            ? toReadableDate(endSlot.time * ONE_THOUSAND, noMillisFormat)
            : '-',
          slotsUsed:
            Math.round(
              ((epoch.details.canonical +
                epoch.details.orphaned +
                epoch.details.missed) /
                epoch.details.totalSlots) *
                100,
            ) + '%',
        };
        this.detect();
      ...
      filter(Boolean),
      filter(epoch => epoch.slots?.length > 0 && !!epoch.details),
    );
  }

  private listenToAllTimeStats(): void {
    this.select(
      BlockProductionOverviewSelectors.allTimeStats,
  ...
  private listenToActiveSlot(): void {
    this.select(BlockProductionOverviewSelectors.activeSlot, slot => {
      ...
      if (slot) {
        this.activeScreen = 1;
        this.activeSlotStatus =
          BlockProductionOverviewSlotsComponent.getSlotText(this.activeSlot);
        this.activeSlotColor =
          BlockProductionOverviewSlotsComponent.getSlotColor(this.activeSlot, {
            canonical: true,
            orphaned: true,
            future: true,
            missed: true,
  ...
  removeActiveSlot(): void {
    this.dispatch2(
      BlockProductionOverviewActions.setActiveSlot({ slot: undefined }),
    );
    this.router.navigate(
      [Routes.BLOCK_PRODUCTION, Routes.OVERVIEW, this.activeEpoch.epochNumber],
