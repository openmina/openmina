...
export class NodesBootstrapOverviewComponent
  ...
  implements OnInit
{
  ...
  @ViewChild('minaBarGraph1', { read: ViewContainerRef })
  private minaBarGraphRef1: ViewContainerRef;
  private component1: BarGraphComponent;
  @ViewChild('minaBarGraph2', { read: ViewContainerRef })
  private minaBarGraphRef2: ViewContainerRef;
  ...
  private listenToNodes(): void {
    this.select(
      ...
      (nodes: NodesBootstrapNode[]) => {
        this.nodes = nodes;
        this.fetchedBlocksTotal = nodes.reduce(
          (acc, n) => acc + n.fetchedBlocks,
          0,
        );
        this.fetchedBlocksAvg =
          nodes.reduce((acc, n) => acc + n.fetchedBlocksAvg, 0) / nodes.length;
        this.fetchedBlocksMax = nodes.reduce(
          (acc, n) => (acc > n.fetchedBlocksMax ? acc : n.fetchedBlocksMax),
          0,
        );
        this.fetchedBlocksMin = nodes.reduce(
          (acc, n) => (acc < n.fetchedBlocksMin ? acc : n.fetchedBlocksMin),
          0,
        );
        this.appliedBlocksTotal = nodes.reduce(
          (acc, n) => acc + n.appliedBlocks,
          0,
        );
        this.appliedBlocksAvg =
          nodes.reduce((acc, n) => acc + n.appliedBlocksAvg, 0) / nodes.length;
        this.appliedBlocksMax = nodes.reduce(
          (acc, n) => (acc > n.appliedBlocksMax ? acc : n.appliedBlocksMax),
          0,
        );
        this.appliedBlocksMin = nodes.reduce(
          (acc, n) => (acc < n.appliedBlocksMin ? acc : n.appliedBlocksMin),
          0,
        );
        this.component1.values = nodes
          .map(n => n.fetchedBlocksAvg)
          .filter(Boolean);
        this.component1.update();
        this.component1.detect();
        this.component2.values = nodes
          .map(n => n.appliedBlocksAvg)
          .filter(Boolean);
