...
export class NodesOverviewTableComponent
  extends MinaTableRustWrapper<NodesOverviewNode>
  implements OnInit
{
  protected readonly tableHeads: TableColumnList<NodesOverviewNode> = [
    ...
    {
      name: 'height',
      tooltip: 'The block height on which the Snarker is currently working. ',
    },
    {
      name: 'best tip',
      sort: 'bestTip',
      tooltip:
        "The blockchain's latest block with the highest known chain strength.",
    },
    {
      name: 'datetime',
      sort: 'bestTipReceivedTimestamp',
      tooltip: 'The date when the block was received.',
    },
    {
      name: 'applied',
      sort: 'appliedBlocks',
      tooltip:
        'Number of blocks that node has applied with the latest synchronization attempt.',
    },
    {
      name: 'fetching',
      sort: 'fetchingBlocks',
      tooltip:
        'Number of blocks that node is currently fetching with the latest synchronization attempt.',
    },
    {
      name: 'fetched',
      sort: 'fetchedBlocks',
      tooltip:
        'Number of blocks that node has fetched with the latest synchronization attempt.',
    },
    {
      name: 'missing blocks',
      sort: 'missingBlocks',
      tooltip:
        'Number of blocks that the node needs to fetch with the latest synchronization attempt.',
    },
  ];
  ...
  protected override setupTable(): void {
    this.table.gridTemplateColumns = [
      100, 180, 100, 100, 95, 130, 165, 120, 120, 120, 150,
    ];
  ...
  private scrollToElement(): void {
    const finder = (node: NodesOverviewNode) =>
      node.name === this.nodeFromRoute;
  ...
  protected override onRowClick(row: NodesOverviewNode): void {
    if (this.table.activeRow?.name !== row?.name) {
      ...
      this.router.navigate([Routes.NODES, Routes.OVERVIEW, row.name], {
