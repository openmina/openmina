...
export class NodesOverviewService {
  ...
  getNodeTips(
    nodeParam: { url: string; name: string },
    qp: string = '',
    ...
    return forkJoin([
      ...
      map((response: [any, NodeDetailsResponse]) =>
        this.mapNodeTipsResponse(response, nodeParam),
      ),
      catchError(() => this.mapNodeTipsErrorResponse(nodeParam)),
    );
  }

  public mapNodeTipsErrorResponse(nodeParam: {
    url: string;
    name: string;
  }): Observable<NodesOverviewNode[]> {
    return of([
  ...
  public mapNodeTipsResponse(
    [syncTips, nodeDetails]: [any, NodeDetailsResponse | undefined],
    nodeParam: {
      url: string;
      name: string;
    },
    ...
    return syncTips.map((node: any) => {
      const blocks = node.blocks.map(
        (block: any) =>
          ({
      ...
      return {
        name: nodeParam.name,
        kind: nodeDetails
          ? nodeDetails.transition_frontier.sync.phase
          : hasValue(node.synced)
            ? NodesOverviewNodeKindType.SYNCED
            : node.kind,
        ...
        appliedBlocks: node.blocks.filter(
          (block: any) => block.status === NodesOverviewNodeBlockStatus.APPLIED,
        ).length,
        applyingBlocks: node.blocks.filter(
          (block: any) =>
            block.status === NodesOverviewNodeBlockStatus.APPLYING,
        ).length,
        missingBlocks: node.blocks.filter(
          (block: any) => block.status === NodesOverviewNodeBlockStatus.MISSING,
        ).length,
        fetchedBlocks: node.blocks.filter(
          (block: any) => block.status === NodesOverviewNodeBlockStatus.FETCHED,
        ).length,
        fetchingBlocks: node.blocks.filter(
          (block: any) =>
            block.status === NodesOverviewNodeBlockStatus.FETCHING,
        ).length,
  ...
  private getLedgers(ledgers: any): NodesOverviewLedger {
    ...
    if (!ledgers.root) {
      ...
      ledger.rootStaged = {
        state:
          !ledgers.root.staged.fetch_parts_start &&
          !ledgers.root.staged.reconstruct_start
        ...
        staged: {
          ...
          fetchPartsDuration: this.getDuration(
            ledgers.root.staged.fetch_parts_start,
            ledgers.root.staged.fetch_parts_end,
          ),
          reconstructDuration: this.getDuration(
            ledgers.root.staged.reconstruct_start,
            ledgers.root.staged.reconstruct_end,
          ),
          fetchPassedTime: this.getPassed(
            ledgers.root.staged.fetch_parts_start,
          ),
          reconstructPassedTime: this.getPassed(
            ledgers.root.staged.reconstruct_start,
          ),
        },
        totalTime: null,
      ...
      if (ledger.rootStaged.state !== NodesOverviewLedgerStepState.PENDING) {
        ...
        ledger.rootSnarked.totalTime =
          ledger.rootSnarked.snarked.fetchHashesDuration +
          ledger.rootSnarked.snarked.fetchAccountsDuration;
      }
      if (ledger.rootStaged.staged.reconstructEnd) {
        ledger.rootStaged.state = NodesOverviewLedgerStepState.SUCCESS;
        ledger.rootStaged.totalTime =
          ledger.rootStaged.staged.fetchPartsDuration +
          ledger.rootStaged.staged.reconstructDuration;
      }
    }
    ...
    if (ledger.rootSnarked.state !== NodesOverviewLedgerStepState.PENDING) {
      ...
      ledger.nextEpoch.totalTime =
        ledger.nextEpoch.snarked.fetchHashesDuration +
        ledger.nextEpoch.snarked.fetchAccountsDuration;
    }
    ...
    if (ledger.nextEpoch.state !== NodesOverviewLedgerStepState.PENDING) {
      ...
      ledger.stakingEpoch.totalTime =
        ledger.stakingEpoch.snarked.fetchHashesDuration +
        ledger.stakingEpoch.snarked.fetchAccountsDuration;
    }
  ...
  private getSnarkedLedgerStep(step: any): NodesOverviewLedgerEpochStep {
    ...
    return {
      state: this.noneOfStepsCompleted(step)
        ? NodesOverviewLedgerStepState.PENDING
        : NodesOverviewLedgerStepState.LOADING,
      snarked: {
        ...
        fetchHashesDuration: this.getDuration(
          step.snarked.fetch_hashes_start,
          step.snarked.fetch_hashes_end,
        ),
        fetchAccountsDuration: this.getDuration(
          step.snarked.fetch_accounts_start,
          step.snarked.fetch_accounts_end,
        ),
        fetchHashesPassedTime: this.getPassed(step.snarked.fetch_hashes_start),
        fetchAccountsPassedTime: this.getPassed(
          step.snarked.fetch_accounts_start,
        ),
      },
    } as NodesOverviewLedgerEpochStep;
  ...
  private noneOfStepsCompleted(step: any): boolean {
    return (
      !step.snarked.fetch_hashes_start && !step.snarked.fetch_accounts_start
    );
  }
}
