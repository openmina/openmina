...
export function reducer(
  state: NodesOverviewState = initialState,
  action: NodesOverviewActions,
): NodesOverviewState {
  switch (action.type) {
    case NODES_OVERVIEW_GET_NODE_STATUS_SUCCESS: {
      ...
      return {
        ...
        activeNode: state.activeNode
          ? nodes.find(node => node.name === state.activeNode.name)
          : undefined,
      };
    }
...
function sortNodes(
  ...
  tableSort: TableSort<NodesOverviewNode>,
): NodesOverviewNode[] {
