...
const select = <T>(
  ...
  createSelector(selectDashboardState, selector);

export const selectDashboardState =
  createFeatureSelector<DashboardState>('dashboard');
export const selectDashboardPeers = select(
  (state: DashboardState): DashboardPeer[] => state.peers,
);
export const selectDashboardPeersStats = select(
  (state: DashboardState): DashboardPeersStats => state.peersStats,
);
export const selectDashboardPeersSort = select(
  (state: DashboardState): TableSort<DashboardPeer> => state.peersSort,
);
export const selectDashboardNodes = select(
  (state: DashboardState): NodesOverviewNode[] => state.nodes,
);
export const selectDashboardNodesAndPeers = select(
  (state: DashboardState): [NodesOverviewNode[], DashboardPeer[]] => [
    state.nodes,
    state.peers,
  ],
);
export const selectDashboardNodesAndRpcStats = select(
  (state: DashboardState): [NodesOverviewNode[], DashboardRpcStats] => [
    state.nodes,
    state.rpcStats,
  ],
);
