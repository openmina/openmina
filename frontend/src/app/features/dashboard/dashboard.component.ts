  ...
  implements OnInit, OnDestroy
{
  ...
  private resetTimer(): void {
    ...
    const timerInterval =
      this.lastStatus === AppNodeStatus.SYNCED ||
      this.lastStatus === AppNodeStatus.OFFLINE
        ? 5000
        : 1000;
    ...
    this.select(
      ...
      ([nodes, rpcStats]: [NodesOverviewNode[], DashboardRpcStats]) => {
        if (nodes.length !== 0) {
          ...
          const fetched = blocks.filter(
            b =>
              ![
                NodesOverviewNodeBlockStatus.MISSING,
                NodesOverviewNodeBlockStatus.FETCHING,
              ].includes(b.status),
          ).length;
          const fetchedPercentage = Math.round((fetched * 100) / 291);

          const applied = blocks.filter(
