...
export class MemoryResourcesEffects extends MinaRustBaseEffect<MemoryResourcesActions> {
  ...
  constructor(
    ...
    this.getResources$ = createEffect(() =>
      this.actions$.pipe(
        ...
        switchMap(({ action, state }) =>
          action.type === MEMORY_RESOURCES_CLOSE
            ? EMPTY
            : this.memoryResourcesService.getStorageResources(
                state.resources.memory.granularity,
              ),
        ),
        map((payload: MemoryResource) => ({
          type: MEMORY_RESOURCES_GET_SUCCESS,
          payload,
        })),
        catchErrorAndRepeat(
    ...
    this.setGranularity$ = createEffect(() =>
      this.actions$.pipe(
        ofType(MEMORY_RESOURCES_SET_GRANULARITY),
        map((payload: MemoryResource) => ({
          type: MEMORY_RESOURCES_GET,
          payload,
        })),
      ),
    );
