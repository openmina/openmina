...
export class ScanStateService {
  ...
  private mapScanState(response: any): ScanStateBlock {
    ...
    const trees: ScanStateTree[] = response.scan_state
      .reverse()
      .map((tree: any[], treeIndex: number) => {
        return {
          availableJobs: tree.filter(
            leaf => leaf.status === ScanStateLeafStatus.Todo,
          ).length,
          ongoing: tree.filter(leaf => leaf.commitment && !leaf.snark).length,
          notIncludedSnarks: tree.filter(
            leaf => leaf.snark && leaf.status === ScanStateLeafStatus.Pending,
          ).length,
          completedSnarks: tree.filter(
            leaf => leaf.snark && leaf.status === ScanStateLeafStatus.Done,
          ).length,
          leafs: tree.map((t, jobIndex: number) => ({
            ...t,
            treeIndex,
            jobIndex,
          })),
    ...
    const workingLeafs = trees.flatMap(tree =>
      tree.leafs.filter(l => l.commitment && !l.snark),
    );

    this.snarkers = this.snarkers.map(
      s =>
        ({
          ...s,
          local: s.url === this.rust.URL,
          leafs: [],
        }) as ScanStateWorkingSnarker,
    );

    workingLeafs.forEach((leaf: ScanStateLeaf) => {
      const snarker = this.snarkers.find(
        s => s.hash === leaf.commitment.commitment.snarker,
      );
      if (snarker) {
