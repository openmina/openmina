...
export class NetworkBlocksService {
  ...
  getBlockMessages(height: number): Observable<NetworkBlock[]> {
    return this.http
      .get<any>(this.config.DEBUGGER + '/block/' + height)
      .pipe(map((blocks: any) => this.mapBlocks(blocks)));
  }
  ...
  private mapBlocks(blocks: any): NetworkBlock[] {
    ...
    const allTimestamps = blocks.events.map((block: any) =>
      this.getTimestamp(block.time),
    );
    const fastestTime: bigint = allTimestamps
      .map((t: string) => BigInt(t))
      .reduce((t1: bigint, t2: bigint) => (t2 < t1 ? t2 : t1));
    return blocks.events.map(
      (block: any, i: number) =>
        ({
          ...
          hash: block.hash,
          date: toReadableDate(
            block.time.secs_since_epoch * ONE_THOUSAND +
              block.time.nanos_since_epoch / ONE_MILLION,
          ),
          ...
          [block.incoming ? 'receivedLatency' : 'sentLatency']:
            Number(BigInt(allTimestamps[i]) - fastestTime) / ONE_BILLION,
        }) as NetworkBlock,
    );
  }
