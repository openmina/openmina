...
export class NetworkNodeDhtTableComponent
  extends MinaTableRustWrapper<NetworkNodeDhtPeer>
  implements OnInit
{
  protected readonly NetworkNodeDhtPeerConnectionType =
    NetworkNodeDhtPeerConnectionType;
  ...
  private scrollToElement(): void {
    if (this.idFromRoute) {
      this.table.scrollToElement(
        request => request.peerId === this.idFromRoute,
      );
      this.setActiveRow(
        this.table.rows.find(request => request.peerId === this.idFromRoute),
      );
  ...
  protected override onRowClick(row: NetworkNodeDhtPeer): void {
    if (this.table.activeRow?.peerId !== row?.peerId) {
      ...
      this.router.navigate([Routes.NETWORK, Routes.NODE_DHT, row.peerId], {
