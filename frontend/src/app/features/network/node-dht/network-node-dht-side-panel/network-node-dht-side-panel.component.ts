...
export class NetworkNodeDhtSidePanelComponent
  ...
  implements OnInit
{
  ...
  private listenToActiveNode(): void {
    this.select(AppSelectors.activeNode, node => {
      this.hasBootstrapStatsEnabled = isSubFeatureEnabled(
        node,
        'network',
        'bootstrap-stats',
      );
  ...
  closeSidePanel(): void {
    ...
    this.router.navigate([Routes.NETWORK, Routes.NODE_DHT], {
  ...
  goToBootstrapStats(): void {
    this.router.navigate(
      [Routes.NETWORK, Routes.BOOTSTRAP_STATS, this.activePeer.peerId],
      { queryParamsHandling: 'merge' },
    );
  }
}
