...
export class NetworkBootstrapStatsEffects extends MinaRustBaseEffect<NetworkBootstrapStatsActions> {
  ...
  constructor(
    ...
    this.getBootstrapStats$ = createEffect(() =>
      this.actions$.pipe(
        ofType(
          NETWORK_BOOTSTRAP_STATS_GET_BOOTSTRAP_STATS,
          NETWORK_BOOTSTRAP_STATS_CLOSE,
        ...
        this.latestActionState<
          NetworkBootstrapStatsGetBootstrapStats | NetworkBootstrapStatsClose
        >(),
        filter(
          ({ action }) =>
        ...
        tap(({ action }) => {
          this.pendingRequest =
            action.type === NETWORK_BOOTSTRAP_STATS_GET_BOOTSTRAP_STATS;
        }),
        switchMap(({ action }) =>
          action.type === NETWORK_BOOTSTRAP_STATS_CLOSE
            ? EMPTY
