...
export class NetworkMessagesSidePanelComponent
  ...
  implements AfterViewInit
{
  ...
  private listenToActiveRowChange(): void {
    this.select(selectNetworkActiveRow, (activeRow: NetworkMessage) => {
      ...
      if (!activeRow) {
        ...
        this.activeRowFullMessage =
          this.activeRowHex =
          this.activeRowHexDisplayedValue =
          this.connection =
            undefined;
      }
      this.detect();
    ...
    this.select(
      ...
      (hex: string) => {
        ...
        this.activeRowHexDisplayedValue =
          NetworkMessagesSidePanelComponent.getActiveRowHexDisplayedValue(hex);
        this.setToCopy();
        this.detect();
      },
      filter(Boolean),
  ...
  private static getActiveRowHexDisplayedValue(hex: string): string {
    return hex.length < 600
      ? hex
      : hex.slice(0, 500) + '...' + hex.slice(hex.length - 9);
  }

  downloadJson(): void {
    const fileName =
      this.selectedTabIndex === 2 ? 'message_hex.txt' : 'network_data.json';
  ...
  expandEntireJSON(): void {
    ...
    this.expandingTracking = this.minaJsonViewer.toggleAll(
      this.userDidHitExpandAll,
    );
  }
