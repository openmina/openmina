...
export const networkAvailableFilters: NetworkMessagesFilterCategory[][] = [
  ...
  [
    {
      name: '/noise',
      tooltip:
        'The Noise Protocol is a framework for building cryptographic protocols',
      filters: [
        {
    ...
    },
    {
      name: '/coda/kad/1.0.0',
      tooltip:
        'Used for peer discovery. The Kademlia Distributed Hash Table (DHT) is a DHT implementation largely based on the Kademlia whitepaper, augmented with notions from S/Kademlia, Coral and the BitTorrent DHT.',
      filters: [
        {
          type: NetworkMessagesFilterTypes.MESSAGE_KIND,
          display: 'Put Value',
        ...
        },
        {
          ...
          value: 'get_value',
          tooltip:
            'Value retrieval, Getting a value by its key from the nodes closest to that key.',
        },
        {
          type: NetworkMessagesFilterTypes.MESSAGE_KIND,
          ...
          value: 'add_provider',
          tooltip:
            'Adding oneself to the list of providers for a given key at the nodes closest to that key by finding the closest nodes.',
        },
        {
          type: NetworkMessagesFilterTypes.MESSAGE_KIND,
          ...
          value: 'get_providers',
          tooltip:
            'Getting providers for a given key from the nodes closest to that key.',
        },
        {
          type: NetworkMessagesFilterTypes.MESSAGE_KIND,
          display: 'Find Node',
          value: 'find_node',
          tooltip: 'Finding the closest nodes to a given key.',
        },
        {
          type: NetworkMessagesFilterTypes.MESSAGE_KIND,
          display: 'Ping',
          value: 'ping',
          tooltip:
            'Deprecated. Message type replaced by the dedicated ping protocol.',
        },
      ],
    },
  ],
  [
    ...
    },
    {
      ...
      tooltip:
        'Bitswap is a module provided by libp2p that enables distributed data synchronization over a p2p network, somewhat comparable to how BitTorrent works.',
      filters: [
        {
          type: NetworkMessagesFilterTypes.STREAM_KIND,
          ...
          tooltip:
            "Using BitSwap, Mina nodes can offset load from GossipSub by gossiping only the header of the block, Splitting the block body into parts and Downloading different parts of the block's body.",
        },
      ],
    },
    {
      ...
      tooltip:
        'NodeStatus is maintained by libp2p_helper process, which is updated by main Mina process using SetNodeStatus call.',
      filters: [
        {
          type: NetworkMessagesFilterTypes.STREAM_KIND,
          display: 'Node Status',
          ...
          tooltip:
            'As soon as you open substream on this protocol, you will receive a json encoded NodeStatus.',
        },
      ],
    },
  ],
      ...
      filters: [
        {
          ...
          value: '/ipfs/id/1.0.0',
          tooltip:
            'The protocol works by opening a stream to the remote peer you want to query. The peer being identified responds by returning an Identify message and closing the stream.',
        },
      ],
    },
    {
      ...
      filters: [
        {
          ...
          tooltip:
            'Responds to identify request by pushing the full identity info to the peer.',
        },
      ],
    },
    {
      ...
      filters: [
        {
          ...
          tooltip:
            'Used for providing changes (only diff) in the list of protocols that the peer supports.',
        },
      ],
    },
  ],
  ...
  [
    {
      ...
      tooltip:
        'RPC - Remote Procedure Call. In some scenarios, a node needs to obtain information from other nodes it is connected to. It makes a particular request to a peer over the P2P network and then expects a response from it.',
      filters: [
        {
          type: NetworkMessagesFilterTypes.MESSAGE_KIND,
        ...
        },
        {
          ...
          tooltip:
            'Returns sparse ledger, or, in case of a failure, a string description.',
        },
        {
          type: NetworkMessagesFilterTypes.MESSAGE_KIND,
          display: 'Versioned rpc menu',
          value: '__Versioned_rpc.Menu',
          tooltip: '',
        },
        {
          type: NetworkMessagesFilterTypes.MESSAGE_KIND,
...
export class NetworkMessagesFiltersComponent
  ...
  implements OnInit
{
  ...
  private addFiltersToRoute(): void {
    const stream_kind = this.activeFilters
      .filter(f => f.type === NetworkMessagesFilterTypes.STREAM_KIND)
      .map(f => f.value)
      .join(',');
    const message_kind = this.activeFilters
      .filter(f => f.type === NetworkMessagesFilterTypes.MESSAGE_KIND)
      .map(f => f.value)
      .join(',');
    const addr = this.activeFilters.find(
      f => f.type === NetworkMessagesFilterTypes.ADDRESS,
    )?.value;
    const queryParams = {
  ...
  filterByCategory(category: NetworkMessagesFilterCategory): void {
    ...
    const type = filters.every(f => this.activeFilters.includes(f))
      ? 'remove'
      : 'add';
    this.dispatch(NetworkMessagesToggleFilter, { filters, type });
  }
