...
export class NetworkMessagesEffects extends MinaRustBaseEffect<NetworkMessagesActions> {
  ...
  constructor(
    ...
    this.getMessages$ = createEffect(() =>
      this.actions$.pipe(
        ...
        map((payload: NetworkMessage[]) => ({
          type: NETWORK_GET_MESSAGES_SUCCESS,
          payload,
        })),
        catchErrorAndRepeat(
    ...
    this.getSpecificMessage$ = createEffect(() =>
      this.actions$.pipe(
        ...
        switchMap(({ action, state }) =>
          forkJoin([
            ...
            map((messages: [NetworkMessage[], NetworkMessage[]]) => ({
              messages: [...messages[0], ...messages[1].slice(1)],
        ...
        switchMap((response: { messages: NetworkMessage[]; id: number }) => [
          { type: NETWORK_GET_MESSAGES_SUCCESS, payload: response.messages },
          {
            ...
            payload: response.messages.find(m => m.id === response.id),
          },
        ]),
    ...
    this.setActiveRow$ = createEffect(() =>
      this.actions$.pipe(
        ...
        switchMap((state: NetworkMessagesState) => {
          switch (state.activeTab) {
            case 1:
              return state.activeRowFullMessage
                ? EMPTY
                : of({
                    type: NETWORK_GET_FULL_MESSAGE,
                    payload: { id: state.activeRow.id },
                  });
            case 2:
              return state.activeRowHex
                ? EMPTY
                : of({
                    type: NETWORK_GET_MESSAGE_HEX,
                    payload: { id: state.activeRow.id },
                  });
            default:
              return state.connection
                ? EMPTY
                : of({
                    type: NETWORK_GET_CONNECTION,
                    payload: { id: state.activeRow.connectionId },
                  });
          }
        }),
      ),
    ...
    this.getFullMessage$ = createEffect(() =>
      this.actions$.pipe(
        ofType(NETWORK_GET_FULL_MESSAGE),
        this.latestActionState<NetworkMessagesGetFullMessage>(),
        mergeMap(({ action }) =>
          this.networkMessagesService.getNetworkFullMessage(action.payload.id),
        ),
        map((payload: any) => ({
          type: NETWORK_GET_FULL_MESSAGE_SUCCESS,
          payload,
        })),
        catchError((err: Error) =>
          of({ type: NETWORK_GET_FULL_MESSAGE_SUCCESS, payload: err.message }),
        ),
        repeat(),
      ),
    );

    this.getMessageHex$ = createEffect(() =>
      this.actions$.pipe(
        ofType(NETWORK_GET_MESSAGE_HEX),
        this.latestActionState<NetworkMessagesGetMessageHex>(),
        switchMap(({ action }) =>
          this.networkMessagesService.getNetworkMessageHex(action.payload.id),
        ),
        map((payload: string) => ({
          type: NETWORK_GET_MESSAGE_HEX_SUCCESS,
          payload,
        })),
        catchErrorAndRepeat(
          MinaErrorType.DEBUGGER,
    ...
    this.getConnection$ = createEffect(() =>
      this.actions$.pipe(
        ofType(NETWORK_GET_CONNECTION),
        this.latestActionState<NetworkMessagesGetConnection>(),
        switchMap(({ action }) =>
          this.networkMessagesService.getNetworkConnection(action.payload.id),
        ),
        map((payload: NetworkMessageConnection) => ({
          type: NETWORK_GET_CONNECTION_SUCCESS,
          payload,
        })),
        catchErrorAndRepeat(
          MinaErrorType.DEBUGGER,
    ...
    this.pause$ = createNonDispatchableEffect(() =>
      this.actions$.pipe(
    ...
    this.goLive$ = createNonDispatchableEffect(() =>
      this.actions$.pipe(
