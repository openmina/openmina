...
export class FuzzingDirectoriesTableComponent
  extends MinaTableRustWrapper<FuzzingDirectory>
  implements OnInit
{
    ...
    this.select(
      ...
      (directories: FuzzingDirectory[]) => {
        ...
        if (this.pathFromRoute) {
          const payload = directories.find(
            dir => dir.fullName === this.pathFromRoute,
          );
