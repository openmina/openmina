...
export class FuzzingCodeComponent
  ...
  implements OnInit, AfterViewInit
{
  ...
  private listenToFileChanges(): void {
    zip(
      this.store
        .select(selectFuzzingActiveFile)
        .pipe(filter(file => this.file !== file)),
      this.store
        .select(selectFuzzingActiveFileDetails)
        .pipe(filter(file => this.fileDetails !== file)),
    )
      ...
      .subscribe(([file, details]) => {
        ...
        if (this.lineToScroll) {
          this.detect();
          this.codeHolder.nativeElement.scrollTo(
            0,
            (Number(this.lineToScroll) - 1) * 24,
          );
  ...
  private listenToMouseMove(): void {
    fromEvent(this.codeContainer.nativeElement, 'mousemove')
      .pipe(
        untilDestroyed(this),
