...
export class StateActionsSidePanelComponent
  extends MinaTableRustWrapper<StateActionGroup>
  implements OnInit
{
  activeSlot: number;
  ...
  private listenToSlotChange(): void {
    this.select(
      selectStateActionsActiveSlotAndStats,
      ([activeSlot, stats]: [number, StateActionsStats]) => {
  ...
  private listenToSort(): void {
    ...
    const tableHeads = this.document.querySelectorAll(
      'mina-table .row.head span',
    );
    this.select(
      selectStateActionsSort,
      (sort: TableSort<StateActionGroup>) => {
        const activeSortColumnIndex = this.tableHeads.findIndex(
          h => h.sort === sort.sortBy,
        );
