...
export class BlockProductionPillComponent
  ...
  implements OnInit, OnDestroy
{
  ...
  private listenToActiveNode(): void {
    this.select(
      ...
      (details: AppNodeDetails) => {
        if (
          details.producingBlockStatus ===
          BlockProductionWonSlotsStatus.Committed
        ) {
          this.text = 'active';
        } else if (
          details.producingBlockStatus ===
          BlockProductionWonSlotsStatus.Produced
        ) {
          this.text = 'done';
        } else {
          this.text = null;
        ...
        this.producingIn = getTimeDiff(this.producingValue, {
          only1unit: true,
        }).diff;
        this.detect();
      },
    ...
    this.select(
      ...
      (route: MergedRoute) => {
        this.activeSubMenu = route.url.split('/')[2]
          ? removeParamsFromURL(route.url.split('/')[2])
          : null;
      },
      filter(Boolean),
    );
  ...
  goToWonSlots(): void {
    ...
    if (this.activeSubMenu === Routes.WON_SLOTS) {
      this.dispatch2(
        BlockProductionWonSlotsActions.setActiveSlotNumber({
          slotNumber: this.globalSlot,
        }),
      );
      return;
    }
    this.router.navigate(
      [Routes.BLOCK_PRODUCTION, Routes.WON_SLOTS, this.globalSlot],
