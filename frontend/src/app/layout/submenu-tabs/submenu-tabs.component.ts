...
export class SubmenuTabsComponent extends StoreDispatcher implements OnInit {
  ...
  private horizontalMenuComponent: HorizontalMenuComponent;
  ...
  private setSubMenusOfActiveNodeForNewPage(node: MinaNode): void {
    const feature = getAvailableFeatures(node || ({} as MinaNode)).find(
      (f: FeatureType) => f === this.baseRoute,
    );
    if (node && node.features) {
      this.subMenus = this.getSubMenusMap(node.features[feature]) || [];
    } else {
      this.subMenus =
        this.getSubMenusMap(CONFIG.globalConfig?.features[feature]) || [];
    }
