...
import {
  MARK_ERRORS_AS_SEEN,
  MarkErrorsAsSeen,
} from '@error-preview/error-preview.actions';
...
export class ErrorPreviewComponent extends ManualDetection implements OnInit {
  ...
  private listenToNewErrors(): void {
    this.store
      .select(selectErrorPreviewErrors)
      ...
      .subscribe((errors: MinaError[]) => {
        if (errors.length !== this.errors.length) {
          ...
          if (
            !errors[1] ||
            (errors[1].seen && errors[0].message !== errors[1].message)
          ) {
  ...
  openErrorList(anchor: HTMLSpanElement, event: MouseEvent): void {
    ...
    this.errorListComponent =
      this.overlayRef.attach<ErrorListComponent>(portal);
    this.errorListComponent.instance.errors = this.errors;
    this.errorListComponent.instance.onConfirm.pipe(take(1)).subscribe(() => {
      this.detachOverlay();
