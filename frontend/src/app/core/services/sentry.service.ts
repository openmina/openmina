...
export class SentryService {
    ...
    if (ledger.rootStaged.state === NodesOverviewLedgerStepState.SUCCESS) {
      ...
      const syncDetails = {
        stakingLedger: {
          ...
          fetchAccounts:
            ledger.stakingEpoch.snarked.fetchAccountsDuration + 's',
        },
        nextEpochLedger: {
      ...
      const syncedIn = Math.round(
        (ledger.rootStaged.staged.reconstructEnd -
          ledger.stakingEpoch.snarked.fetchHashesStart) /
          ONE_BILLION,
      );
  ...
  updateBlockSyncStatus(
    ...
    const blocksSynced = blocks.every(
    ...
    if (blocksSynced && blocks[0]) {
      ...
      Sentry.captureMessage(
        `Last 290 blocks synced in ${getElapsedTime(this.blockSyncedTime)}`,
        {
          level: 'info',
          tags: {
            type: 'webnode',
      ...
      setTimeout(() => {
        Sentry.captureMessage(
          `Web Node Synced in ${getElapsedTime(syncTotal)}`,
          {
            level: 'info',
            tags: {
              type: 'webnode',
  ...
  updateProducedBlock(
    attempt: BlockProductionAttempt,
    publicKey: string,
  ): void {
    const times = {
      stagedLedgerDiffCreate:
        ...
        !attempt.times.staged_ledger_diff_create_start
          ...
          : (attempt.times.staged_ledger_diff_create_end -
            ...
            ONE_BILLION,
      produced:
        !attempt.times.produced || !attempt.times.staged_ledger_diff_create_end
          ? 0
          : (attempt.times.produced -
              attempt.times.staged_ledger_diff_create_end) /
            ONE_BILLION,
      proofCreate:
        !attempt.times.proof_create_end || !attempt.times.proof_create_start
          ? 0
    ...
    Sentry.captureMessage(
      `Block Production Finished (${attempt.status}) - ` +
        attempt.block?.height,
      {
        level: 'info',
        tags: {
          type: 'webnode',
          subType: 'block.production',
          publicKey,
          duration:
            times.stagedLedgerDiffCreate + times.produced + times.proofCreate,
        },
        fingerprint: this.fingerprint,
    ...
    Sentry.captureMessage('Heartbeat', {
      ...
      contexts: {
        heartbeat: {
          ...
          signatureField: data.signature.field,
          signatureScalar: data.signature.scalar,
        },
      },
      fingerprint: this.fingerprint,
    });
