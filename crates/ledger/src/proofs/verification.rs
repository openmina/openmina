...
pub fn prev_evals_from_p2p<F: FieldWitness>(
    ...
    let of = |(zeta, zeta_omega): &(_, _)| of(zeta, zeta_omega);
    ...
    ProofEvaluations {
        ...
        lookup_sorted: crate::array_into_with(lookup_sorted, of_opt),
...
pub fn prev_evals_to_p2p(
    ...
    let of = |PointEvaluations { zeta, zeta_omega }: &PointEvaluations<Vec<Fp>>| {
        (
            zeta.iter().map(|v| BigInt::from(*v)).collect(),
            zeta_omega.iter().map(|v| BigInt::from(*v)).collect(),
        )
    };
...
///
/// This function validates that a block header contains a valid zero-knowledge proof
/// by performing two checks:
/// 1. **Accumulator check**: Validates the polynomial commitment opening proofs
/// 2. **Proof verification**: Verifies the blockchain SNARK proof against the protocol state hash.
///    The blockchain SNARK proves the validity of the protocol state transaction (consensus). Defined
///    in the OCaml codebase at
///    <https://github.com/MinaProtocol/mina/tree/compatible/src/lib/blockchain_snark>.  This function
///    verifies proofs generated by this circuit.
///
...
pub fn verify_block(
    ...
    let protocol_state = ProtocolState::from(protocol_state);
...
pub fn verify_transaction<'a>(
    ...
    let accum_check = accumulator_check::accumulator_check(srs, &accum_check_proofs);
