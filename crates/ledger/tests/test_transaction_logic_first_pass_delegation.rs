//! Tests for apply_transaction_first_pass with stake delegation transactions
//!
//! Run with: cargo test --test test_transaction_logic_first_pass_delegation
//!
//! Tests the first pass of two-phase transaction application for stake
//! delegations, covering:
//! - Successful delegation to a new delegate
//! - Delegation with insufficient balance for fee
//! - Delegation with invalid nonce
//! - Delegation from nonexistent fee payer
...
use mina_tree::{
    scan_state::{
        ...
        transaction_logic::{
            ...
            signed_command::{
                Body, Common, SignedCommand, SignedCommandPayload, StakeDelegationPayload,
            },
...
fn create_test_ledger() -> Mask {
    ...
    // Create Bob's account (potential delegate)
...
fn create_delegation(
    ...
    let payload = SignedCommandPayload {
        ...
        body: Body::StakeDelegation(StakeDelegationPayload::SetDelegate {
            new_delegate: to_pk.clone(),
        }),
    };
...
#[test]
fn test_apply_delegation_success() {
    ...
    let initial_alice_delegate = alice_before.delegate.clone();

    let fee = 10_000_000;
    let nonce = 0;
    let delegation = create_delegation(&alice_pk, &bob_pk, fee, nonce);
    ...
    // Verify Alice's balance decreased by fee only (no amount transferred in
    // delegation)
    ...
    // Verify Alice's delegate was set to Bob
    assert_eq!(
        alice_after.delegate.as_ref(),
        Some(&bob_pk),
        "Alice's delegate should be set to Bob"
    );
    assert_ne!(
        alice_after.delegate, initial_alice_delegate,
        "Alice's delegate should have changed"
    );

    // Verify Bob's balance unchanged (he's the delegate, not receiving funds)
    assert_eq!(
        bob_after.balance, initial_bob_balance,
        "Bob's balance should remain unchanged"
    );

    // Verify Bob's nonce unchanged
    assert_eq!(
...
fn test_apply_delegation_insufficient_balance() {
    ...
    let alice_id = AccountId::new(alice_pk.clone(), Default::default());
    let alice_account = Account::create_with(alice_id.clone(), Balance::from_u64(1_000_000));
    ledger
    ...
    // Verify ledger state remains unchanged
    let alice_location = ledger.location_of_account(&alice_id).unwrap();
    let alice_after = ledger.get(alice_location).unwrap();
    ...
    assert_eq!(
        alice_after.delegate, initial_alice_delegate,
        "Alice's delegate should remain unchanged"
    );
}

/// Test delegation with incorrect nonce.
///
/// The transaction is rejected during fee payment validation because the
...
/// Ledger state: Remains unchanged (no fee charged, no nonce incremented).
#[test]
fn test_apply_delegation_invalid_nonce() {
    let mut ledger = create_test_ledger();
...
/// Test delegation from a nonexistent fee payer account.
///
/// The transaction is rejected during fee payment validation because the
...
#[test]
fn test_apply_delegation_nonexistent_fee_payer() {
    let db = Database::create(15);
    let mut ledger = Mask::new_root(db);
