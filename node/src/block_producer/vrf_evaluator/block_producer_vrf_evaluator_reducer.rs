    ...
    pub fn reducer(&mut self, action: BlockProducerVrfEvaluatorActionWithMetaRef<'_>) {
        ...
        match action {
            BlockProducerVrfEvaluatorAction::EpochDataUpdate(action) => {
                self.evaluator_status = BlockProducerVrfEvaluatorStatus::EpochChanged { time: meta.time() };
                ...
                self.current_epoch_data.total_currency = action.epoch_data.ledger.total_currency.as_u64();
            ...
            BlockProducerVrfEvaluatorAction::EvaluateVrf(action) => {
                // self.evaluator_status = BlockProducerVrfEvaluatorStatus::Pending(action.vrf_input.global_slot);
                self.evaluator_status = BlockProducerVrfEvaluatorStatus::SlotsRequested { time: meta.time() };
            }
            ...
            BlockProducerVrfEvaluatorAction::EvaluationSuccess(action) => {
                let global_slot_evaluated = match &action.vrf_output {
                    vrf::VrfEvaluationOutput::SlotWon(won_slot_data) => {
                        self.won_slots.insert(won_slot_data.global_slot, won_slot_data.clone());
                ...
                self.evaluator_status = BlockProducerVrfEvaluatorStatus::SlotsReceived { time: meta.time() };
                self.latest_evaluated_slot = global_slot_evaluated;
            },
            BlockProducerVrfEvaluatorAction::UpdateProducerAndDelegates(_) => {
                self.evaluator_status = BlockProducerVrfEvaluatorStatus::DataPending { time: meta.time() };
            },
            BlockProducerVrfEvaluatorAction::UpdateProducerAndDelegatesSuccess(action) => {
                self.evaluator_status = BlockProducerVrfEvaluatorStatus::DataSuccess { time: meta.time() };
                self.current_epoch_data.delegator_table = action.current_epoch_producer_and_delegators.clone();
                self.next_epoch_data.delegator_table = action.current_epoch_producer_and_delegators.clone();
            },
