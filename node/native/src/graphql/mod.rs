//! # GraphQL API Module
//!
//! Implements a comprehensive GraphQL API for the OpenMina node, providing a web-accessible interface for querying blockchain state and submitting transactions. [Ref: file:node/native/src/graphql/mod.rs (0.95)]
//!
//! ## Core Responsibilities
//!
//! - **Schema Definition**: Defines a GraphQL schema with queries and mutations that expose node functionality. [Ref: file:node/native/src/graphql/mod.rs::routes (0.9)]
//! - **Query Resolution**: Resolves GraphQL queries by fetching data from the node's state machine via RPC. [Ref: item:node/native/src/graphql/mod.rs::Query (0.95)]
//! - **Transaction Submission**: Processes mutations that submit transactions to the node's mempool. [Ref: item:node/native/src/graphql/mod.rs::Mutation (0.95)]
//! - **Data Conversion**: Converts between internal node data structures and GraphQL-compatible types. [Ref: multi:(account.rs, block.rs, zkapp.rs) (0.9)]
//! - **HTTP Integration**: Exposes the GraphQL API via HTTP endpoints using Warp. [Ref: item:node/native/src/graphql/mod.rs::routes (0.95)]
//!
//! ## Key Components
//!
...
//! - **Error Handling**: Converts internal errors to GraphQL-compatible error responses. [Ref: item:node/native/src/graphql/mod.rs::Error (0.9)]
...
//!
//! The GraphQL API serves as the primary interface for external applications to interact with the OpenMina node,
//! providing a flexible and well-documented way to query blockchain data and submit transactions. [Ref: doc:/Users/lukasimrich/Code/GitHub/openmina/docs/components-overview.md (0.8)]
