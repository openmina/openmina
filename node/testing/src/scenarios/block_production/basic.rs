...
impl BlockProductionBasic {
    pub async fn run(self, mut runner: ClusterRunner<'_>) {
        ...
        genesis_mask.iter(|acc| {
            ...
            let delegate = acc.delegate.as_ref().map_or(pub_key.clone(), |pk| pk.into_address());
        ...
        let (chain_id, ocaml_node_peer_id) = {
            ...
            let chain_id = node.chain_id().expect("failed to get chain_id from ocaml node");
        ...
        runner.run(RunCfg::default().timeout(Duration::from_secs(60)).action_handler(move |id, _, _, action| {
            id == seed_node &&
            matches!(action.action().kind(), ActionKind::TransitionFrontierGenesisInject)
        })).await.expect("seed node didn't produce genesis block");

        runner.run(RunCfg::default().timeout(Duration::from_secs(60)).action_handler(move |id, state, _, action| {
            dbg!(action.action().kind());
            id == seed_node && 
                state.p2p.get_ready_peer(&ocaml_node_peer_id).map_or(false, |peer| peer.best_tip.is_some())
        })).await.expect("seed node didn't produce genesis block");

        // make sure genesis blocks are the same.
        self.assert_all_nodes_best_tip_eq(&runner);
    }

    async fn assert_all_nodes_best_tip_eq(&self, runner: &ClusterRunner<'_>) {
        eprintln!("[check_tips]");
        for (id, node) in runner.ocaml_nodes_iter() {
            let tip =node.synced_best_tip_async().await.expect("failed to get best tip from ocaml node").expect("ocaml node not synced");
            eprintln!("{id:?} - {tip}");
        }
