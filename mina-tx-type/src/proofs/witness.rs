use super::{field::FieldWitness, to_field_elements::ToFieldElements};

/// Trait for checking values in the witness.
pub trait Check<F: FieldWitness> {
    fn check(&self, w: &mut Witness<F>);
}

/// Trait for proof constants.
...
impl<F: FieldWitness> Witness<F> {
    ...
    // Note: The helper methods that depend on transaction module functions
    // (to_field_checked_prime, add_fast) are not included here as they would
    // create circular dependencies. They remain in the ledger crate.
}
