...
protocols. Such peers are Rust-node based and `rust-libp2p` based (to have
reference implementatios).

Running and observing a test cluster is done by reading it as a `Stream`,
emitting different events as its components progress.
