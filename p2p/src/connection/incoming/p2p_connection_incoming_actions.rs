...
impl redux::EnablingCondition<P2pState> for P2pConnectionIncomingErrorAction {
    fn is_enabled(&self, state: &P2pState) -> bool {
        state
            ...
            .get(&self.peer_id)
            .map_or(false, |peer| match &peer.status {
                P2pPeerStatus::Connecting(P2pConnectionState::Incoming(v)) => match v {
                    P2pConnectionIncomingState::FinalizeError { error, .. } => error == &self.error,
                    _ => false,
                },
                _ => false,
            })
...
use super::{IncomingSignalingMethod, P2pConnectionIncomingState};
