        ...
        Store::Service: P2pCryptoService,
    {
        match self {
            P2pNetworkPubsubEffectfulAction::Sign { author, topic } => {
                handle_sign(author, topic, store);
            }
            P2pNetworkPubsubEffectfulAction::IncomingData {
                peer_id,
                seen_limit,
            } => {
                handle_incoming_data(peer_id, seen_limit, store);
            }
        }
    }
}
...
#[inline(never)]
fn handle_sign<Store, S>(author: PeerId, topic: String, store: &mut Store)
where
    Store: crate::P2pStore<S>,
    Store::Service: P2pCryptoService,
{
...
#[inline(never)]
fn handle_incoming_data<Store, S>(peer_id: PeerId, seen_limit: usize, store: &mut Store)
where
    Store: crate::P2pStore<S>,
    Store::Service: P2pCryptoService,
{
    ...
    for message in messages {
        ...
        if let Err(error) = result {
            let Some((addr, _)) = store.state().network.scheduler.find_peer(&peer_id) else {
